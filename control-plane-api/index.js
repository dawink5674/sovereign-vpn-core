...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
      return res.status(400).json({ error: 'Invalid public key: must be a valid base64 encoded 32-byte Curve25519 key' });
    }
...
app.delete('/api/peers/:publicKey', async (req, res) => {
  ...
  // Validate decoded base64 key is 44 chars (32 bytes base64-encoded)
