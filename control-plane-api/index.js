...
if (!process.env.API_KEY) {
  console.error('CRITICAL: API_KEY environment variable is not set. Exiting securely.');
  process.exit(1);
}
const API_KEY_BUFFER = Buffer.from(process.env.API_KEY);
...
function requireAuth(req, res, next) {
  ...
  if (!crypto.timingSafeEqual(providedBuffer, API_KEY_BUFFER)) {
    return res.status(401).json({ error: 'Unauthorized: Invalid API Key' });
  }
  next();
}
