...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    // Strict regex validation for base64 Curve25519 public key (44 chars exactly)
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
      return res.status(400).json({
        error: 'Invalid public key format: must be exactly 44 base64 characters',
      });
    }

    // Validate base64 key is 32 bytes base64-encoded
    const keyBuffer = Buffer.from(publicKey, 'base64');
    if (keyBuffer.length !== 32) {
      return res.status(400).json({
        error: 'Invalid public key: must decode to 32 bytes (Curve25519)',
      });
    }
...
app.delete('/api/peers/:publicKey', async (req, res) => {
  ...
  // Strict regex validation for base64 Curve25519 public key
