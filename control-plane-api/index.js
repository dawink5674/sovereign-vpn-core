...
async function applyPeerToServer(publicKey, presharedKey, assignedIP) {
  if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
    return { success: false, error: 'Strict validation failed: invalid public key format' };
  }
...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    // Validate base64 format strictly to prevent injection
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
      return res.status(400).json({
        error: 'Invalid public key format: must be a valid 44-character base64 string',
      });
    }
