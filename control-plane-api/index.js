...
async function applyPeerToServer(publicKey, presharedKey, assignedIP) {
  try {
    ...
    if (!WG_KEY_REGEX.test(publicKey) || !WG_KEY_REGEX.test(presharedKey)) {
...
async function removePeerFromServer(publicKey) {
  try {
    const WG_KEY_REGEX = /^[A-Za-z0-9+/]{43}=$/;
    if (!WG_KEY_REGEX.test(publicKey)) {
      throw new Error('Invalid key format');
    }
...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    const WG_KEY_REGEX = /^[A-Za-z0-9+/]{43}=$/;
    if (!WG_KEY_REGEX.test(publicKey)) {
      return res.status(400).json({ error: 'Invalid public key: must be 32 bytes (Curve25519)' });
    }
