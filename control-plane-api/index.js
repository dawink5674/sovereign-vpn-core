...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    // Validate base64 key is exactly 44 chars and valid format (32 bytes base64-encoded)
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
      return res.status(400).json({
        error: 'Invalid public key: must be 32 bytes base64-encoded (Curve25519)',
      });
    }
...
app.delete('/api/peers/:publicKey', async (req, res) => {
  ...
  // Validate base64 key format before proceeding
