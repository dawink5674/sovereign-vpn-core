...
async function applyPeerToServer(publicKey, presharedKey, assignedIP) {
  try {
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) throw new Error("Invalid publicKey format");
    if (!/^[A-Za-z0-9+/]{43}=$/.test(presharedKey)) throw new Error("Invalid presharedKey format");
...
async function removePeerFromServer(publicKey) {
  try {
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) { throw new Error('Invalid public key format'); }
