...
function requireAuth(req, res, next) {
  ...
  const adminApiKeyBuffer = Buffer.from(adminApiKey, 'utf-8');
  const providedKeyBuffer = Buffer.from(providedKey, 'utf-8');

  if (adminApiKeyBuffer.length !== providedKeyBuffer.length || !crypto.timingSafeEqual(adminApiKeyBuffer, providedKeyBuffer)) {
