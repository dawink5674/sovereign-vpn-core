...
async function applyPeerToServer(publicKey, presharedKey, assignedIP) {
  if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
    console.error(`⚠️  SSH apply failed: Invalid public key format for injection prevention`);
...
async function removePeerFromServer(publicKey) {
  if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
    console.error(`⚠️  SSH remove failed: Invalid public key format for injection prevention`);
    return { success: false, error: 'Invalid public key format' };
  }
...
app.post('/api/peers', async (req, res) => {
  try {
    ...
    // Strict regex validation for WireGuard base64 public key (prevents injection and confirms format)
    if (!/^[A-Za-z0-9+/]{43}=$/.test(publicKey)) {
      return res.status(400).json({ error: 'Invalid public key format' });
    }

    // Validate base64 key is exactly 32 bytes when decoded
...
app.delete('/api/peers/:publicKey', async (req, res) => {
  ...
  // Strict regex validation for WireGuard base64 public key
