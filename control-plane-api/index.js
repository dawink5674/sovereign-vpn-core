...
const requireAuth = (req, res, next) => {
  if (!API_KEY) {
    console.error('Server is missing API_KEY environment variable.');
    return res.status(500).json({ error: 'Internal Server Error: Server misconfiguration' });
  }
  ...
  const providedKey = Buffer.from(apiKeyHeader);
